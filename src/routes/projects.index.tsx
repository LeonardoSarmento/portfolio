...
import { projectsQueryOptions, projectsQueryOptionsWithFilter, useQueryTags } from '@services/hooks/postsQueryOptions';
import { useSuspenseQuery } from '@tanstack/react-query';
import { Link, createFileRoute } from '@tanstack/react-router';
import { FilterSchema, FilterType, NOT_FOUND } from './posts.index';
...
export const Route = createFileRoute('/projects/')({
  ...
  loader: ({ context: { queryClient }, deps: { tags, title, count, views } }) =>
    queryClient.ensureQueryData(projectsQueryOptionsWithFilter({ tags, title, count, views })),
          ...
          <div className="col-span-11 col-start-2 grid h-fit w-full grid-cols-12 gap-4">
            ...
            {projects.length > 0 ? (
              projects.map((project, index) => (
                <Card key={`${project.id}-${index}`} className="col-span-2 row-span-1 mt-12 p-2 text-center">
                  <Link
